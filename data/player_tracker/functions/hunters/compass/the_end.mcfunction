...
execute in minecraft:overworld run data modify storage hnr:compass Items[].tag.LodestoneDimension set value "minecraft:the_end"

#> updates x, y and z positions of the compass
execute in minecraft:overworld run execute store result storage hnr:compass Items[].tag.LodestonePos.X int 1 run scoreboard players get @e[team=runners,limit=1,sort=nearest,nbt={Dimension:1}] hnr.xpos
execute in minecraft:overworld run execute store result storage hnr:compass Items[].tag.LodestonePos.Y int 1 run scoreboard players get @e[team=runners,limit=1,sort=nearest,nbt={Dimension:1}] hnr.ypos
execute in minecraft:overworld run execute store result storage hnr:compass Items[].tag.LodestonePos.Z int 1 run scoreboard players get @e[team=runners,limit=1,sort=nearest,nbt={Dimension:1}] hnr.zpos
execute in minecraft:overworld run data merge block 0 2 1 {Text1:'{"selector":"@e[team=runners,limit=1,sort=nearest,nbt={Dimension:1}]"}'}
